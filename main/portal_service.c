...
static esp_err_t status_handler(httpd_req_t *req)
{
    ...
    cJSON_AddNumberToObject(root, "expected_file_size", (double)rec.expected_file_size);
    cJSON_AddNumberToObject(root, "record_id", rec.record_id);
    cJSON_AddNumberToObject(root, "active_record_id", rec.active_record_id);
    cJSON_AddNumberToObject(root, "duration_ms", rec.duration_ms);
    cJSON_AddNumberToObject(root, "sample_rate_hz", rec.sample_rate_hz);
    cJSON_AddStringToObject(root, "record_path", rec.path);
    cJSON_AddStringToObject(root, "download_name", rec.download_name);
    err = send_json(req, root);
    cJSON_Delete(root);
    return err;
}
...
static esp_err_t download_record_handler(httpd_req_t *req)
{
    ...
    snprintf(record_size, sizeof(record_size), "%u", (unsigned)st.st_size);
    snprintf(record_id, sizeof(record_id), "%lu", (unsigned long)rec.record_id);
    snprintf(disposition, sizeof(disposition), "attachment; filename=\"%s\"", rec.download_name);
    httpd_resp_set_hdr(req, "Content-Disposition", disposition);
    httpd_resp_set_hdr(req, "Cache-Control", "no-store");
    httpd_resp_set_hdr(req, "X-Record-Id", record_id);
    httpd_resp_set_hdr(req, "X-Record-Size", record_size);
    httpd_resp_set_hdr(req, "X-Record-Path", rec.path);
    ESP_LOGI(TAG,
        "[HTTP] GET /api/record.wav id=%lu path=%s size=%u expected=%u name=%s",
        (unsigned long)rec.record_id,
        path,
        (unsigned)st.st_size,
        (unsigned)rec.expected_file_size,
        rec.download_name);
